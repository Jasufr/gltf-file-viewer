...
const Menu = (props) => {
  const { handleOnChange, handleOnSelect, modelInputValue, modelSelectValue, selectedFile, setSelectedFile, environment, setEnvironment } = props;
  ...
  const preloadedEnvironments = ["brown_photostudio", "leadenhall_market", "ninomaru_teien", "pure_sky", "rosendal_park_sunset"];
  preloadedEnvironments.sort();
        ...
        <div className="flex-1 flex items-start mt-20 flex-col gap-6 p-8">
          ...
          <div className="text-slate-50 py-1 text-lg font-semibold">Preloaded Models</div>
            <div className="text-slate-50 flex items-center border-2 border-slate-500 rounded-s-md bg-slate-900">
              ...
              <select defaultValue={"selected"} className="text-slate-500 bg-slate-50 py-1 px-2 w-full truncate" name="model" id="model-select" onChange={(e) => {handleOnSelect(e.target.value); setSelectedFile("Select a model"); setMenuOpened(!menuOpened); modelInputValue.value = '';}}>
          ...
          <div className="text-slate-50 flex items-center border-2 border-slate-500 rounded-s-md bg-slate-900">
            ...
            <select defaultValue={"none"} name="environment" id="environment-select" className="text-slate-500 bg-slate-50 py-1 px-2 w-full truncate" onChange={(e) => { setEnvironment(e.target.value) }}>
